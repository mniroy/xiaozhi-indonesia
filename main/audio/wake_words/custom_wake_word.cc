      ...
      for (int i = 0; i < cJSON_GetArraySize(commands); i++) {
        ...
        if (cJSON_IsObject(command)) {
          ...
          if (cJSON_IsString(command_name) && cJSON_IsString(text) &&
            ...
            commands_.push_back({command_name->valuestring, text->valuestring,
                                 action->valuestring});
            ESP_LOGI(TAG, "Command: %s, Text: %s, Action: %s",
                     command_name->valuestring, text->valuestring,
                     action->valuestring);
          }
        }
      }
...
bool CustomWakeWord::Initialize(AudioCodec *codec,
  ...
  if (models_list == nullptr) {
    ...
    commands_.push_back(
        {CONFIG_CUSTOM_WAKE_WORD, CONFIG_CUSTOM_WAKE_WORD_DISPLAY, "wake"});
#endif
  } else {
    models_ = models_list;
  ...
  if (mn_name_ == nullptr) {
    ESP_LOGW(
        TAG,
        "Language '%s' multinet not found, falling back to any multinet model",
        language_.c_str());
    mn_name_ = esp_srmodel_filter(models_, ESP_MN_PREFIX, NULL);
  }
  if (mn_name_ == nullptr) {
    ESP_LOGE(TAG, "Failed to initialize multinet, mn_name is nullptr");
    ESP_LOGI(TAG, "Please refer to "
                  "https://pcn7cs20v8cr.feishu.cn/wiki/"
                  "CpQjwQsCJiQSWSkYEvrcxcbVnwh to add custom wake word");
    return false;
  }
...
void CustomWakeWord::Stop() { running_ = false; }
...
void CustomWakeWord::Feed(const std::vector<int16_t> &data) {
  ...
  if (mn_state == ESP_MN_STATE_DETECTING) {
    ...
    for (int i = 0; i < mn_result->num && running_; i++) {
      ESP_LOGI(TAG,
               "Custom wake word detected: command_id=%d, string=%s, prob=%f",
