...
static uint8_t *convert_input_to_encoder_buf(const uint8_t *src, uint16_t width,
  ...
  // V4L2 UYVY (Cb Y Cr Y) -> Rearranged to YUYV then input as YCbYCr
  // UYVY format does not appear in current version temporarily
  ...
  // YUV422P format does not appear in current version temporarily
  ...
  // See https://github.com/78/xiaozhi-esp32/issues/1380#issuecomment-3497156378
    ...
    case V4L2_PIX_FMT_RGB565:
      ...
      break;
    [[unlikely]] case V4L2_PIX_FMT_RGB565X: // RGB565X format does not appear in
                                            // current version temporarily
...
static uint8_t *convert_input_to_hw_encoder_buf(
  ...
  if (format == V4L2_PIX_FMT_YUYV) {
    // Hardware requires | Y1 V Y0 U | "big endian" format, so bswap16 is needed
    int sz = (int)width * (int)height * 2;
