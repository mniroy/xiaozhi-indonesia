...
DualNetworkBoard::DualNetworkBoard(gpio_num_t ml307_tx_pin,
                                   gpio_num_t ml307_rx_pin,
                                   gpio_num_t ml307_dtr_pin,
                                   int32_t default_net_type)
    : Board(), ml307_tx_pin_(ml307_tx_pin), ml307_rx_pin_(ml307_rx_pin),
      ml307_dtr_pin_(ml307_dtr_pin) {

  // Load network type from Settings
  network_type_ = LoadNetworkTypeFromSettings(default_net_type);

  // Only initialize board corresponding to current network type
  InitializeCurrentBoard();
}

NetworkType
DualNetworkBoard::LoadNetworkTypeFromSettings(int32_t default_net_type) {
  Settings settings("network", true);
  int network_type =
      settings.GetInt("type", default_net_type); // Default use ML307 (1)
...
void DualNetworkBoard::InitializeCurrentBoard() {
  if (network_type_ == NetworkType::ML307) {
    ...
    current_board_ = std::make_unique<Ml307Board>(ml307_tx_pin_, ml307_rx_pin_,
                                                  ml307_dtr_pin_);
  } else {
