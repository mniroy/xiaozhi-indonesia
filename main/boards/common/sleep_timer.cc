...
SleepTimer::SleepTimer(int seconds_to_light_sleep, int seconds_to_deep_sleep)
    : seconds_to_light_sleep_(seconds_to_light_sleep),
      seconds_to_deep_sleep_(seconds_to_deep_sleep) {
    ...
    if (!in_light_sleep_mode_) {
      ...
      app.Schedule([this, &app]() {
        while (in_light_sleep_mode_) {
          ...
          // Configure timer wakeup source (wakeup automatically after 30
          // seconds)
          ...
          ESP_LOGI(TAG, "Wake up from light sleep, wakeup_reason: %d",
                   wakeup_reason);
