...
#define CAM_PRINT_FOURCC(pixelformat)                                          \
  ...
  fourcc[4] = '\0';                                                            \
  ESP_LOGD(TAG, "FOURCC: '%c%c%c%c'", fourcc[0], fourcc[1], fourcc[2],         \
           fourcc[3]);
...
Esp32Camera::Esp32Camera(const esp_video_init_config_t &config) {
  ...
  if (false) { /* Used to build else if */
  }
  ...
  if (video_fd_ < 0) {
    ESP_LOGE(TAG, "open %s failed, errno=%d(%s)", video_device_name, errno,
             strerror(errno));
#if CONFIG_XIAOZHI_ENABLE_CAMERA_DEBUG_MODE
  ...
  if (ioctl(video_fd_, VIDIOC_QUERYCAP, &cap) != 0) {
    ESP_LOGE(TAG, "VIDIOC_QUERYCAP failed, errno=%d(%s)", errno,
             strerror(errno));
    close(video_fd_);
    video_fd_ = -1;
    return;
  ...
  // Note: In current version of esp_video, YUV422P actually outputs YUYV.
#if defined(CONFIG_XIAOZHI_ENABLE_ROTATE_CAMERA_IMAGE) &&                      \
    defined(CONFIG_SOC_PPA_SUPPORTED)
  auto get_rank = [](uint32_t fmt) -> int {
    ...
    case V4L2_PIX_FMT_YUV420: // Software JPEG encoder does not support YUV420
                              // format
      return 2;
  ...
  while (ioctl(video_fd_, VIDIOC_ENUM_FMT, &fmtdesc) == 0) {
    ESP_LOGD(TAG, "VIDIOC_ENUM_FMT: pixelformat=0x%08lx, description=%s",
  ...
  ESP_LOGD(TAG, "selected pixel format: 0x%08lx",
           setformat.fmt.pix.pixelformat);
  ...
  // Allocate buffers and mmap
  struct v4l2_requestbuffers req = {};
  req.count =
      strcmp(video_device_name, ESP_VIDEO_MIPI_CSI_DEVICE_NAME) == 0 ? 2 : 1;
  ...
  // When ISP is enabled, ISP needs some photos to initialize parameters, so
  // take photos in background for 5s and discard them after opening
  xTaskCreate(
      [](void *arg) {
        ...
        ESP_LOGI(TAG, "Camera init success, captured %d frames in %dms",
                 capture_count,
...
void Esp32Camera::SetExplainUrl(const std::string &url,
...
bool Esp32Camera::Capture() {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (i == 2) {
      ...
      if (!frame_.data) {
        ESP_LOGE(TAG, "alloc frame copy failed: need allocate %lu bytes",
                 buf.bytesused);
        if (ioctl(video_fd_, VIDIOC_QBUF, &buf) != 0) {
      ...
      case V4L2_PIX_FMT_RGB565X: {
        ...
        // Currently esp_video returns RGB565 for both big and little endian, it
        // will not return RGB565X, this case is for future version
        // compatibility
        auto src16 = (uint16_t *)mmap_buffers_[buf.index].start;
        auto dst16 = (uint16_t *)frame_.data;
      ...
      esp_imgfx_err_t imgfx_err =
          esp_imgfx_rotate_open(&rotate_cfg, &rotate_handle);
      ...
      imgfx_err = esp_imgfx_rotate_process(rotate_handle, &rotate_input_data,
                                           &rotate_output_data);
      ...
      case V4L2_PIX_FMT_RGB24:
        ...
        break;
      case V4L2_PIX_FMT_YUYV: {
        ESP_LOGW(TAG, "YUYV format is not supported for PPA rotation, using "
                      "software conversion to RGB888");
        rotate_src =
            (uint8_t *)heap_caps_malloc(frame_.width * frame_.height * 3,
        ...
        err = esp_imgfx_color_convert_process(
            convert_handle, &convert_input_data, &convert_output_data);
  ...
  if (display != nullptr) {
    ...
    switch (frame_.format) {
    // LVGL seems to have issues displaying YUV family images, temporarily
    // convert to RGB565 for display
    case V4L2_PIX_FMT_YUYV:
    case V4L2_PIX_FMT_YUV420:
    case V4L2_PIX_FMT_RGB24: {
      color_format = LV_COLOR_FORMAT_RGB565;
      data = (uint8_t *)heap_caps_malloc(w * h * 2,
                                         MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
      if (data == nullptr) {
    ...
    case V4L2_PIX_FMT_RGB565:
      ...
      lvgl_image_size =
          frame_.len; // Handle both YUYV and RGB565 in fallthrough
      break;
    ...
    case V4L2_PIX_FMT_JPEG: {
      ...
      esp_err_t ret =
          jpeg_to_image(frame_.data, frame_.len, &out_data, &out_len,
    ...
    auto image = std::make_unique<LvglAllocatedImage>(data, lvgl_image_size, w,
                                                      h, stride, color_format);
...
/**
 * @brief Sends the image captured by the camera to a remote server for AI
 * analysis and explanation
 *
 * This function encodes the image in the current camera buffer into JPEG format
 * and sends it to the specified explanation server via an HTTP POST request in
 * multipart/form-data format. The server analyzes the image based on the
 * provided question and returns the result.
 *
 * Implementation features:
 * - Uses a separate thread for JPEG encoding, separated from the main thread
 * - Uses chunked transfer encoding to optimize memory usage
 * - Uses a queue mechanism to synchronize data between the encoding thread and
 * the sending thread
 * - Supports HTTP header configuration for device ID, client ID, and
 * authentication token
 *
 * @param question The question to ask the AI about the image, sent as a form
 * field
 * @return std::string JSON format response string returned by the server
 *         Contains AI analysis result on success, error message on failure
 *         Format example: {"success": true, "result": "Analysis result"}
 *                         {"success": false, "message": "Error message"}
 *
 * @note SetExplainUrl() must be called to set the server URL before calling
 * this function
 * @note The function waits for the previous encoding thread to complete before
 * starting new processing
 * @warning Returns an error message if the camera buffer is empty or the
...
std::string Esp32Camera::Explain(const std::string &question) {
  ...
  // Create local JPEG queue, 40 entries is about to store 512 * 40 = 20480
  // bytes of JPEG data
  ...
  // We spawn a thread to encode the image to JPEG using optimized encoder (cost
  // about 500ms and 8KB SRAM)
  ...
  if (http->GetStatusCode() != 200) {
    ESP_LOGE(TAG, "Failed to upload photo, status code: %d",
             http->GetStatusCode());
  ...
  ESP_LOGI(TAG,
           "Explain image size=%d bytes, compressed size=%d, remain stack "
           "size=%d, question=%s\n%s",
           (int)frame_.len, (int)total_sent, (int)remain_stack_size,
