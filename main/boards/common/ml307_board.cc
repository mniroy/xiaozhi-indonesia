...
void Ml307Board::NetworkTask() {
  ...
  if (modem_ == nullptr) {
    ESP_LOGE(TAG, "Failed to detect modem after %d retries",
  ...
  // Note: Don't call GetCarrierName() here as it sends AT command and will
  // block ReceiveTask
  ...
  if (!modem_->network_ready()) {
    ESP_LOGE(TAG, "Failed to register network after %d retries",
             NETWORK_REG_MAX_RETRIES);
    return;
  }
...
std::string Ml307Board::GetDeviceStatusJson() {
  ...
  if (audio_codec) {
    cJSON_AddNumberToObject(audio_speaker, "volume",
                            audio_codec->output_volume());
  }
