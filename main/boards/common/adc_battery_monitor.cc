...
AdcBatteryMonitor::AdcBatteryMonitor(adc_unit_t adc_unit,
  ...
  // Set conditions in ADC configuration section
  if (charging_pin_ != GPIO_NUM_NC) {
    ...
    // Do not set callback, let adc_battery_estimation library use software
    // estimation
...
bool AdcBatteryMonitor::IsCharging() {
  // Prioritize using adc_battery_estimation library functions
  if (adc_battery_estimation_handle_ != nullptr) {
    bool is_charging = false;
    esp_err_t err = adc_battery_estimation_get_charging_state(
        adc_battery_estimation_handle_, &is_charging);
    if (err == ESP_OK) {
      return is_charging;
    }
  }

  // Fallback to GPIO reading or return default value
  if (charging_pin_ != GPIO_NUM_NC) {
...
uint8_t AdcBatteryMonitor::GetBatteryLevel() {
  ...
  float capacity = 0;
  esp_err_t err = adc_battery_estimation_get_capacity(
...
void AdcBatteryMonitor::OnChargingStatusChanged(
