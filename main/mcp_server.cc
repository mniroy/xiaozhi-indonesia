...
void McpServer::AddCommonTools() {
  ...
  // **Important** To improve response speed, we place commonly used tools at
  // the front to leverage prompt cache characteristics.
...
void McpServer::GetToolsList(int id, const std::string &cursor,
  ...
  while (it != tools_.end()) {
    // If we haven't found the start position yet, continue searching
    if (!found_cursor) {
      if ((*it)->name() == cursor) {
        found_cursor = true;
      } else {
    ...
    if (json.length() + tool_json.length() + 30 > max_payload_size) {
      // If adding this tool will exceed the size limit, set next_cursor and
      // exit loop
      next_cursor = (*it)->name();
      break;
    }
