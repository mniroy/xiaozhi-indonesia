...
bool Assets::InitializePartition() {
  ...
  if (storage_size < partition_->size) {
    ESP_LOGE(
        TAG,
        "The free size %ld KB is less than assets partition required %ld KB",
  ...
  if (stored_len > partition_->size - 12) {
    ESP_LOGD(TAG,
             "The stored_len (0x%lx) is greater than the partition size "
             "(0x%lx) - 12",
             stored_len, partition_->size);
    return false;
  }
  ...
  ESP_LOGI(TAG, "The checksum calculation time is %d ms",
           int((end_time - start_time) / 1000));

  if (calculated_checksum != stored_chksum) {
    ESP_LOGE(TAG,
             "The calculated checksum (0x%lx) does not match the stored "
             "checksum (0x%lx)",
  ...
  for (uint32_t i = 0; i < stored_files; i++) {
    auto item = (const mmap_assets_table *)(mmap_root_ + 12 +
                                            i * sizeof(mmap_assets_table));
    auto asset = Asset{.size = static_cast<size_t>(item->asset_size),
                       .offset = static_cast<size_t>(
                           12 + sizeof(mmap_assets_table) * stored_files +
                           item->asset_offset)};
...
bool Assets::Apply() {
  ...
  if (cJSON_IsNumber(version)) {
    if (version->valuedouble > 1) {
      ESP_LOGE(
          TAG,
          "The assets version %d is not supported, please upgrade the firmware",
          version->valueint);
      return false;
    }
  }
      ...
      if (cJSON_IsObject(emoji)) {
        ...
        if (cJSON_IsString(name) && cJSON_IsString(file) && (NULL == eaf)) {
          if (!GetAssetData(file->valuestring, ptr, size)) {
            ESP_LOGE(TAG, "Emoji %s image file %s is not found",
  ...
  if (cJSON_IsObject(skin)) {
    ...
    if (cJSON_IsObject(light_skin) && light_theme != nullptr) {
      ...
      cJSON *background_color =
          cJSON_GetObjectItem(light_skin, "background_color");
      cJSON *background_image =
          cJSON_GetObjectItem(light_skin, "background_image");
      if (cJSON_IsString(text_color)) {
        light_theme->set_text_color(
            LvglTheme::ParseColor(text_color->valuestring));
      }
    ...
    if (cJSON_IsObject(dark_skin) && dark_theme != nullptr) {
      ...
      cJSON *background_color =
          cJSON_GetObjectItem(dark_skin, "background_color");
      cJSON *background_image =
          cJSON_GetObjectItem(dark_skin, "background_image");
      if (cJSON_IsString(text_color)) {
        ...
        if (cJSON_IsObject(icon)) {
          ...
          if (cJSON_IsString(name) && cJSON_IsString(file)) {
            if (GetAssetData(file->valuestring, ptr, size)) {
              ...
              ESP_LOGE(TAG, "Emoji \"%10s\" image file %s is not found",
                       name->valuestring, file->valuestring);
            }
          }
        }
        ...
        if (cJSON_IsObject(icon)) {
          ...
          if (cJSON_IsString(name) && cJSON_IsString(file)) {
            if (GetAssetData(file->valuestring, ptr, size)) {
              ...
              ESP_LOGE(TAG, "Icon \"%10s\" image file %s is not found",
                       name->valuestring, file->valuestring);
            }
          }
        }
  ...
  if (cJSON_IsArray(layout_json)) {
    ...
    for (int i = 0; i < layout_count; i++) {
      ...
      if (cJSON_IsObject(layout_item)) {
        ...
        if (cJSON_IsString(name) && cJSON_IsString(align) &&
          ...
          if (emote_display) {
            emote_display->AddLayoutData(name->valuestring, align->valuestring,
                                         x->valueint, y->valueint, width_val,
                                         height_val);
          }
        } else {
...
bool Assets::Download(
  ...
  // Unmap current assets partition memory
  if (mmap_handle_ != 0) {
    esp_partition_munmap(mmap_handle_);
    mmap_handle_ = 0;
    mmap_root_ = nullptr;
  ...
  if (http->GetStatusCode() != 200) {
    ESP_LOGE(TAG, "Failed to get assets, status code: %d",
             http->GetStatusCode());
    return false;
  }
  ...
  size_t sectors_to_erase =
      (content_length + SECTOR_SIZE - 1) / SECTOR_SIZE; // Round up
  size_t total_erase_size = sectors_to_erase * SECTOR_SIZE;

  ESP_LOGI(TAG,
           "Sector size: %u, content length: %u, sectors to erase: %u, total "
           "erase size: %u",
           SECTOR_SIZE, content_length, sectors_to_erase, total_erase_size);

  // Write new assets file to partition, erasing while writing
  char buffer[512];
  ...
  while (true) {
    ...
    if (esp_timer_get_time() - last_calc_time >= 1000000 ||
        total_written == content_length || ret == 0) {
  ...
  ESP_LOGI(TAG,
           "Assets download completed, total written: %u bytes, total sectors "
           "erased: %u",
...
bool Assets::GetAssetData(const std::string &name, void *&ptr, size_t &size) {
  ...
  if (data[0] != 'Z' || data[1] != 'Z') {
    ESP_LOGE(TAG, "The asset %s is not valid with magic %02x%02x", name.c_str(),
             data[0], data[1]);
    return false;
  }
